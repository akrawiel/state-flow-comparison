...
function CommandSelector() {
  ...
  return isInputActive
    ? h(React.Fragment, {}, 
      h(Ink.Text, { dimColor: 'grey' }, 'Select packages to run'),
      h(Ink.Box, { width: '100%' }, 
        h(Ink.Text, { dimColor: 'grey' }, '<tab> to toggle, <enter> to run selected '),
    ...
    : h(React.Fragment, {},
      h(Ink.Text, { color: 'green', bold: true }, 'Running selected packages dev servers... (Ctrl+C to stop)'),
      selectedPackages.map(
        ({ package, name, port }) => h(
          Ink.Text,
          { color: 'blueBright', bold: true, key: package },
          `Dev server for ${name} (${package}) will run at port ${port}`
        ),
      ),
      h(Ink.Newline)
    )
